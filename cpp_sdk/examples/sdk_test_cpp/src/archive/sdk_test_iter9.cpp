...
// Iteration 9: Query table ID by name instead of guessing
...
extern "C" {
    ...
    void log_msg(const char* msg) {
        const char* filename = "sdk_test_iter9.cpp";
    ...
    int16_t __call_reducer__(
        ...
        if (id == 0) {  // insert_one_u8 reducer
            ...
            result = datastore_insert_bsatn(table_id, row_data_copy.data(), &row_len);
            if (result != 0) {
                snprintf(msg, sizeof(msg), "Failed to insert: error code %u", result);
                log_msg(msg);
                
                // Log the row data for debugging
                snprintf(msg, sizeof(msg), "Row data length: %zu", row_data_copy.size());
                log_msg(msg);
                for (size_t i = 0; i < row_data_copy.size() && i < 10; i++) {
                    snprintf(msg, sizeof(msg), "Row data[%zu]: %02x", i, (unsigned)row_data_copy[i]);
                    log_msg(msg);
                }
