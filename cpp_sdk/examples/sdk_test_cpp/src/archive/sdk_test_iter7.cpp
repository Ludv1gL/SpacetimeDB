...
// Iteration 7: Proper reducer implementation with correct arg deserialization
...
extern "C" {
    ...
    void log_msg(const char* msg) {
        const char* filename = "sdk_test_iter7.cpp";
    ...
    int16_t __call_reducer__(
        ...
        if (id == 0) {  // insert_one_u8 reducer
            ...
            // Create row data (single u8 value)
            ...
            uint16_t result = datastore_insert_bsatn(0, row_data, &row_len);
            if (result != 0) {
                ...
                for (uint32_t tid = 1; tid < 10; tid++) {
                    row_len = 1;  // Reset length
                    snprintf(msg, sizeof(msg), "Trying table_id %u", tid);
                    log_msg(msg);
                    
                    result = datastore_insert_bsatn(tid, row_data, &row_len);
                    if (result == 0) {
