...
// Iteration 10: Row is serialized as struct fields directly, not ProductValue
...
extern "C" {
    ...
    void log_msg(const char* msg) {
        const char* filename = "sdk_test_iter10.cpp";
    ...
    int16_t __call_reducer__(
        ...
        if (id == 0) {  // insert_one_u8 reducer
            ...
            uint32_t table_id;
            uint16_t result = table_id_from_name((const uint8_t*)table_name, strlen(table_name), &table_id);
            if (result != 0) {
                char msg[128];
                snprintf(msg, sizeof(msg), "Failed to get table ID for '%s': error code %u", table_name, result);
                log_msg(msg);
                return -1;
            }
            
            char msg[256];
            snprintf(msg, sizeof(msg), "Found table '%s' with ID: %u", table_name, table_id);
            log_msg(msg);
            ...
            // Create row data as BSATN encoding of the struct fields
            // For OneU8 struct { n: u8 }, the BSATN is just the u8 value
            std::vector<uint8_t> row_data;
            row_data.push_back(n);
            
            size_t row_len = row_data.size();
            
            // Insert into table using the queried table ID
            snprintf(msg, sizeof(msg), "Inserting value %u into table_id %u", n, table_id);
            log_msg(msg);
            
            result = datastore_insert_bsatn(table_id, row_data.data(), &row_len);
            if (result != 0) {
                snprintf(msg, sizeof(msg), "Failed to insert: error code %u", result);
                log_msg(msg);
                
                // Log the row data for debugging
                snprintf(msg, sizeof(msg), "Row data length: %zu", row_data.size());
                log_msg(msg);
                for (size_t i = 0; i < row_data.size() && i < 10; i++) {
                    snprintf(msg, sizeof(msg), "Row data[%zu]: %02x", i, (unsigned)row_data[i]);
                    log_msg(msg);
                }
