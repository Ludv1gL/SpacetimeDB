...
// Iteration 4: Fix Option encoding (Some=0, None=1)

extern "C" {
    __attribute__((import_module("spacetime_10.0"), import_name("bytes_sink_write")))
    uint16_t bytes_sink_write(uint32_t sink, const uint8_t* buffer_ptr, size_t* buffer_len_ptr);
    
    // AlgebraicType enum values based on Rust definition
    enum AlgebraicTypeTag : uint8_t {
        AT_Ref = 0,
        AT_Sum = 1,
        AT_Product = 2,
        AT_Array = 3,
    ...
    public:
        ...
        void write_option_some() {
            buffer.push_back(0);  // Some = 0 (not 1!)
        }
        
        void write_option_none() {
            buffer.push_back(1);  // None = 1 (not 0!)
        }
    ...
    void __describe_module__(uint32_t sink) {
        ...
        w.write_u8(AT_Product);  // AlgebraicType::Product = 2
        w.write_u32(1);          // 1 field
        
        // Field: n
        w.write_option_some();   // field has name
        w.write_string("n");
        w.write_u8(AT_U8);       // AlgebraicType::U8 = 7
